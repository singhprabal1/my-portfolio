  ...
  return (
    <span style={{ position: "relative", display: "inline-block" }}>
      <span style={{
        ...
        color: "#f03", opacity: glitch ? 0.7 : 0, userSelect: "none",
        transition: "none",
...
export default function App() {
  ...
  const sectionRefs = { home: useRef(), projects: useRef(), experience: useRef(), contact: useRef() };

  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(e => { if (e.isIntersecting) setActive(e.target.dataset.section); });
    }, { threshold: 0.35 });
    Object.entries(sectionRefs).forEach(([k, r]) => { if (r.current) { r.current.dataset.section = k; observer.observe(r.current); } });
